...
void FileLogger::refreshSettings() {    
    ...
    QString level = settings->value("minLevel", "DEBUG").toString();
    if (level == "DEBUG") minLevel = DebugLevel;
    else if (level == "INFO") minLevel = InfoLevel;
    else if (level == "WARN") minLevel = WarnLevel;
    else if (level == "ERROR") minLevel = ErrorLevel;
    else if (level == "FATAL") minLevel = FatalLevel;
    else minLevel = DebugLevel;
