...
inline LogLevel QtMsgTypeToLogLevel(QtMsgType type)
{
    switch (type) {
    case QtDebugMsg: return DebugLevel;
    case QtInfoMsg: return InfoLevel;
    case QtWarningMsg: return WarnLevel;
    case QtCriticalMsg: return ErrorLevel;
    case QtFatalMsg: return FatalLevel;
    default:
        return DebugLevel;
    }
}
...
public:
    ...
    */
    ...
    /**
      ...
      @param minLevel Minimum severity that genertes an output (0=debug, 1=info, 2=warning, 3=critical, 4=fatal).
      @param bufferSize Size of the backtrace buffer, number of messages per thread. 0=disabled.
      @param parent Parent object
      @see LogMessage for a description of the message decoration.
    */
    Logger(const QString msgFormat="{timestamp} {type} {msg}", const QString timestampFormat="dd.MM.yyyy hh:mm:ss.zzz", const LogLevel minLevel=DebugLevel, const int bufferSize=0, QObject* parent = 0);
