...
Logger::Logger(const QString msgFormat, const QString timestampFormat, const LogLevel minLevel, const int bufferSize, QObject* parent)
    :QObject(parent) {
...
void Logger::log(const QtMsgType type, const QString& message, const QString &file, const QString &function, const int line) {
    ...
    if (bufferSize>0) {
        ...
        if (QtMsgTypeToLogLevel(type)>=minLevel) {
